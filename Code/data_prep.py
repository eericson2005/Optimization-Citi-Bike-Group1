...
print(f"  Stations with at least one pre-10pm snapshot: {first_snap['station_name'].nunique()}")
...
daytime_trips = trips[
    (trips["start_hour"] >= DAY_START_HOUR)
    & (trips["start_hour"] <  DAY_END_HOUR)
].copy()
...
def compute_rates(trip_df, dep_col, arr_col, suffix):
    ...
    daily_dep = (
        trip_df.groupby([dep_col, "date"])
        .size()
        .groupby(dep_col)
        .mean()
    )
    daily_arr = (
        trip_df.groupby([arr_col, "date"])
        .size()
        .groupby(arr_col)
        .mean()
    )
...
print(
    params[["station_name", "kappa", "mu_s_weekday", "lambda_s_weekday",
            "mu_s_weekend", "lambda_s_weekend"]]
    .head(10)
